...
constexpr int32_t FM_DEEMPH_ALPHA_50US_Q31 = 1935044054;
constexpr int32_t FM_DEEMPH_ALPHA_300US_Q31 = 2100413000;
...
public:
    T push(T x, T alpha)
    {
        ...
        state = T(std::clamp<Sum>(mixed >> QFRAC,
...
static uint16_t fm_q31_to_dac12(int32_t y_q31)
{
    ...
    int64_t clamped_q19 = std::clamp<int64_t>(shaped_q19, 0, (int64_t)4095 << 19);
    int32_t dac;
...
static void fm_audio_out_process_frames(const uint16_t* words, size_t frame_count)
{
    ...
    for(size_t i = 0U; i < frame_count; ++i)
    {
        ...
        if(s_has_prev != 0U)
        {
            ...
            dac_hw_stream_fm_push_sample_isr(fm_q31_to_dac12<MODE>(s_deemph.push(audio_cic_q31, deemph_alpha_q31)));
        }
        else
        {
            s_has_prev = 1U;
